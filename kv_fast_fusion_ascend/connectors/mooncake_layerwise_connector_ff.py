...
Redirect maps reach D on a **dedicated PUSH/PULL side channel** of our own (own port, own message
tag, own thread pair: ``_FFRedirectSendThread`` → ``_FFRedirectRecvThread``).

That separation is load-bearing, and was re-learned the hard way. Riding the base connector's
``DONE_SENDING_MSG`` is tempting — it would make "map present before the owner is promoted" an
ordering invariant rather than a race — but it requires reaching into the base's control plane, and
that control plane is NOT stable across vllm-ascend versions. An attempt to do it (2026-08-04)
vendored ``KVCacheRecvingLayerThread.run`` from the source tree on the dev box and called
``update_task(req_id, trans_count)``; the runtime's newer build had grown a third parameter
(``side_channel_path``), so every done-signal raised ``TypeError`` inside the base's blanket
``except``, the ACK was never sent, and NO request was ever marked received on D — the run did not
merely lose compression, it stopped working. The same drift makes the message LAYOUT unsafe to
extend: slot 3 may already be spoken for.

So: never couple redirect delivery to the base's messages, signatures, or thread bodies. The one
base method still wrapped is ``save_kv_layer`` (and it binds its arguments by NAME — see
``_ff_install_worker_hooks``). Delivery ordering is not guaranteed by construction here; it is
MEASURED instead, by ``promo_no_rows`` / ``promo_rows_late`` / ``promo_maps_late`` in
``bff_apply_stats`` — and at con512 it came in at promo_no_rows=2, i.e. the push channel beats the
KV transfer essentially always.
...
# untouched — see the module docstring for why that separation is load-bearing, not incidental.
# TP=1 assumed for M1 (the new setup runs tp_size=1 on both P and D).
...
try:
    ...
    # Deliberately minimal: only the connector base, its metadata/worker types and the external-id
    # helper. Nothing here reaches into the base's control plane (message tags, listener threads,
    # done-signal helpers) — see the module docstring for what that coupling cost.
...
if _ASCEND_AVAILABLE:
    ...
    class _FFRedirectRecvThread(threading.Thread):
        ...
        records each arrived redirect map as ``{external_id: {gi: rows}}`` for the apply path
        (``drain`` / ``_FF_PENDING_SOURCE`` / the scheduler promotion hook). Fire-and-forget: no ACK
        is sent, so the producer never blocks — a dropped map just means that request keeps
        per-request copies on D (less compression, never incorrect).

        Deliberately a channel of OUR OWN — own port, own message tag, own thread — rather than a
        rider on the base connector's DONE_SENDING_MSG. See the module docstring: the base's control
        plane is version-dependent in ways this repo cannot see from a dev box, and coupling to it
        broke the decode node outright."""
