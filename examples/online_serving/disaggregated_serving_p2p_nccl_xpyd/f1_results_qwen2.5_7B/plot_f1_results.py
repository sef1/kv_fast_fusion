...
_BFF_RE = re.compile(
    ...
    r"_thr(?P<thr>[\d.]+)_gs(?P<gs>\d+)_con_(?P<con>\d+)_(?P<np>\d+)Px(?P<nd>\d+)D$")
_VAN_RE = re.compile(r"^vanilla(?:_con_(?P<con>\d+))?_(?P<np>\d+)Px(?P<nd>\d+)D$")


def parse_label(label, fallback_con=None):
    """label → dict of parsed config (merge/scale/repr/thr/gs/con/topology/system)."""
    m = _BFF_RE.match(label)
    if m:
        d = m.groupdict()
        return {
            "system": "bff", "merge": d["merge"], "scale": d["scale"], "repr": d["repr"],
            "thr": float(d["thr"]), "gs": int(d["gs"]), "con": int(d["con"]),
            "topo": f"{d['np']}Px{d['nd']}D",
        }
    ...
    if m:
        ...
        return {
            ...
            "thr": float("nan"), "gs": 0, "con": con, "topo": f"{d['np']}Px{d['nd']}D",
        }
    return {"system": "other", "merge": "-", "scale": "-", "repr": "-",
            "thr": float("nan"), "gs": 0, "con": fallback_con or 0, "topo": "?"}
...
def load_records(directory):
    ...
    for path in sorted(glob.glob(os.path.join(directory, "f1_*.json"))):
        ...
        if p["system"] == "bff":
            rec["tag"] = f"{p['merge']}/{p['scale']}/{p['repr']} thr{p['thr']} gs{p['gs']} c{con} {p['topo']}"
...
# CSV summary
# ---------------------------------------------------------------------------
_CSV_COLS = ["file", "system", "merge", "scale", "repr", "thr", "gs", "con", "topo",
...
def plot_tradeoff(recs, out):
    ...
    for r in recs:
        ...
        note = (f"van c{r['con']}" if van else f"gs{r['gs']} thr{r['thr']} c{r['con']}")
...
def plot_sweeps(recs, out_dir):
    """Auto 1-D sweeps: for each of threshold / group-size / concurrency, plot F1 and req/s
    vs that knob, drawing a line per group of runs that hold all OTHER knobs fixed."""
    bff = [r for r in recs if r["system"] == "bff"]
    dims = {"thr": "threshold", "gs": "group size", "con": "concurrency"}
    for dim, dim_label in dims.items():
        others = [d for d in ("thr", "gs", "con") if d != dim]
...
def main():
    ...
    print("\nPlots:")
    plot_tradeoff(recs, os.path.join(out_dir, "accuracy_vs_throughput.png"))
    plot_headline_bars(recs, os.path.join(out_dir, "headline_metrics.png"))
    plot_latency(recs, os.path.join(out_dir, "latency_breakdown.png"))
    plot_compression_heatmap(recs, os.path.join(out_dir, "bff_compression_per_group.png"))
    plot_decode_capacity(recs, os.path.join(out_dir, "decode_capacity.png"))
    plot_f1_distribution(recs, os.path.join(out_dir, "f1_distribution.png"))
