...
from vllm.v1.worker.compression_hook import CompressionHook
...
class BlockCompressionHook(CompressionHook):  
    ...
    def __init__(self, vllm_config):  
        ...
        self.thr = 0.7
        self._should_compress_layer = [f"model.layers.{i}.self_attn.attn" for i in range(self.vllm_config.model_config.get_num_layers(self.vllm_config.parallel_config))][2:-2]
    ...
    def start_layer_compression(self, layer_name, kv_cache, attn_metadata):  
        ...
        # if not hasattr(self, "_should_compress_layer"):
        #     self._should_compress_layer = list(self.vllm_config.compilation_config.static_forward_context.keys())[self.warmup_layers:-self.warmup_layers]

        # Check if we need to compress this layer  
        if layer_name not in self._should_compress_layer:  
            return
        else:
            B, num_blocks = attn_metadata.block_table.shape
            # if not hasattr(self, "idx__"):
        ...
        with torch.cuda.stream(self.compression_stream):  
            ...
            # self._compress_layer_async(layer_name, kv_cache, attn_metadata)  
...
def execute_model_v1(
        ...
        req_to_compress = [req_id for req_id in self.input_batch.req_ids if req_id not in self.fused_requests]
        updated_block_tables = {}
        if self.compression_hook and len(req_to_compress) > 1 and not is_decode:                     
            ...
            with set_forward_context(  
                    ...
                    compression_hook=self.compression_hook,  # Add this line  
                ):  
            # Model forward pass - each layer will trigger compression 
                ...
                **MultiModalKwargs.as_kwargs(
                ...
                # Wait for all layer compressions to complete  
                ...
                updated_block_tables = self._update_block_tables_after_compression(
                    req_to_compress 
                ) 
        ...
        # Start compression AFTER model forward pass  
