...
from bench_dataset import (
    ...
    generate_tool_conversations,    
    parse_input_json_file,
)
from bench_utils import TEXT_SEPARATOR, Color, logger
from transformers import AutoTokenizer  # type: ignore
from benchmarks.agentic.benchmark_agentic import AgenticBenchmark
...
class ServerResponse(NamedTuple):
    ...
    num_chunks: int # number of output chunks
    tools: list[dict] | None = None  # Tool calls if any
...
class DebugStats:
    def __init__(self, logger: logging.Logger, window_size: int) -> None:
        ...
        self.metrics: dict[str, MovingAverage | MetricStats] = {
            ...
            "tool_calls_count": MetricStats(),
            "tool_calls_correct": MetricStats(),
        }
    ...
    def update(self, data: RequestStats) -> None:
        ...
        self.metrics["tool_calls_count"].update(data.tool_calls_count)
        self.metrics["tool_calls_correct"].update(data.tool_calls_correct)
            ...
            async for chunk_bytes in response.content:
                ...
                if chunk == "[DONE]":
                    ...
                    if "delta" in choice:  
                        ...
                        # Non-streaming response (e.g., tool calls)  
...
async def send_turn_(
    ...
    f"Message missing required keys in conversation ID {conv_id}"  
    )  
    assert messages[index]["role"] == "user", (  
    f"Failed on conversation ID {conv_id}, message role should be user"  
    )
    # assert len(messages[index].keys()) == 2 #sefi
    # assert "role" in messages[index] and "content" in messages[index]
    # assert messages[index]["role"] == "user", (
    #     f"Failed on conversation ID {conv_id}, message role should be user"
    # )
    ...
    if response.valid and hasattr(response, 'tool_calls'):  
        # Analyze tool calls  
        tool_result = AgenticBenchmark._analyze_tool_calls_static(  
            messages, response, send_turn._tool_parser  
        )  
          
        # Update RequestStats with tool metrics  
        rs.tool_calls_count = len(tool_result["tool_calls"])  
        rs.tool_calls_correct = tool_result["correct"]
...
async def send_turn(
    ...
    # Include ALL messages up to current turn (including tool messages)  
    messages = conversation_messages[:messages_to_use]  
  
    # Find the actual user message index for this turn  
    # user_message_index = messages_to_use - 1  
    # while (user_message_index >= 0 and   
    #        conversation_messages[user_message_index]["role"] != "user"):  
    #     user_message_index -= 1  
      
    # assert user_message_index >= 0, (  
    #     f"No user message found for turn {messages_to_use} in conversation {conv_id}"  
    # )  
  
  # Find the last user message in the conversation slice  
    user_message_index = -1  
    for i in range(messages_to_use - 1, -1, -1):  
        if messages[i]["role"] == "user":  
            user_message_index = i  
            break  
      
    assert user_message_index >= 0, (  
        f"No user message found in conversation ID {conv_id} "  
        f"in first {messages_to_use} messages"  
    )  
    # Verify the user message has required fields  
    user_msg = conversation_messages[user_message_index]  
    assert "role" in user_msg and "content" in user_msg  
    assert user_msg["role"] == "user"  
  
    # Extract tools from the conversation if available  
    tools = None  
    for msg in conversation_messages[:messages_to_use]:  
        if "tool_calls" in msg:  
            tools = msg["tool_calls"]  
            break  
  
    # Send request with complete conversation history and tools  
    response: ServerResponse = await send_request(  
        session,  
        messages,  # Complete conversation including tool messages  
        req_args.chat_url,  
        req_args.model,  
        req_args.stream,  
        None,  # min_tokens  
        None,  # max_tokens  
        req_args.timeout_sec,  
        tools,  # Include tools if available  
    )
    ...
    # Num tokens in the user's last question
    question_num_tokens = get_token_count(tokenizer, messages[user_message_index]["content"])
    ...
    rs = RequestStats(
        ...
        conversation_id=conv_id,
        client_id=client_id,
        tool_calls_count=len(response.tools) if response.tools is not None else 0,
        # tool_calls_correct=response.tool_calls_correct,
        # tool_output_tokens=response.tool_output_tokens
    )
    ...
    if len(conversation_messages) > answer_index:
        ...
        # orig_content = conversation_messages[answer_index]["content"]
        # Handle assistant messages that may have tool_calls instead of content  
        assistant_msg = conversation_messages[answer_index]  
        if "tool_calls" in assistant_msg and assistant_msg["tool_calls"]:  
            # For tool call messages, skip content verification or extract from tool calls  
            if verify_output:  
                logger.info(f"Skipping content verification for tool call message in {conv_id}")  
            # Don't update tool call messages - they should preserve their structure  
        else:  
            # Regular assistant message with content  
            orig_content = assistant_msg.get("content", "")  
            if verify_output:  
                # Compare the new answer to the answer from the input file  
                debug_info = (  
                    f"LLM/dataset answers do not match ({conv_id}):"  
            ...
            # Update the answer for regular messages  
            conversation_messages[answer_index]["content"] = output_content
...
async def client_main(
    ...
    # Initialize tool parser once for this client  
    # from benchmarks.agentic.benchmark_agentic import AgenticBenchmark  
    from vllm.entrypoints.openai.tool_parsers import ToolParserManager
    tool_parser_class = ToolParserManager.get_tool_parser("pythonic")
            ...
            for attempt_cnt in range(args.max_retries + 1):
                try:
                    ...
                    result = await send_turn(
                        ...
                        args.print_content,
                        args.verify_output,
                        tool_parser  # Pass tool_parser to send_turn
                    )
...
def process_statistics(
    ...
    if "tool_calls_count" in df.columns:  
        print(f"Tool calls per turn: {df['tool_calls_count'].mean():.2f}")  
        print(f"Tool call accuracy: {(df['tool_calls_correct'].sum() / df['tool_calls_count'].sum() * 100):.1f}%")
...
async def main() -> None:
    ...
    if isinstance(input_data, list):
        ...
        if gen_conv_args.agent_mode:  
            conversations = generate_tool_conversations(gen_conv_args, tokenizer)  
        else:  
