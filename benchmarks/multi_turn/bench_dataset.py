...
class GenConvArgs(NamedTuple):
    ...
    print_stats: bool
    agent_mode: bool = False  # Add this  
    tools: list[dict] = []   # Add this
...
def parse_input_json_file(conf: dict) -> GenConvArgs:
    ...
    # Parse agent_mode and tools  
    agent_mode = conf.get("agent_mode", False)  
    ...
    args = GenConvArgs(
        ...
        agent_mode=agent_mode,  # Add this  
...
# def generate_tool_conversations(args: GenConvArgs, tokenizer: AutoTokenizer) -> ConversationsMap:  
#     """Generate multi-turn conversations with tool calling."""  
#     from benchmarks.agentic.tool_call_dataset import ToolCallDataset  
      
#     # Use your existing ToolCallDataset  
#     dataset = ToolCallDataset("tool_calling")  
#     conversations = {}  
      
#     # Generate test cases with multiple turns  
#     test_cases = dataset.get_test_cases(args.num_conversations)  
      
#     for i, test_case in enumerate(test_cases):  
#         conv_id = f"TOOL_CONV_{i}"  
#         conversations[conv_id] = test_case["messages"]  
      
#     return conversations


# def generate_tool_conversations(args: GenConvArgs, tokenizer) -> ConversationsMap:  
#     """Generate multi-turn conversations with tool calling."""  
#     from benchmarks.agentic.tool_call_dataset import ToolCallDataset  
      
#     # Load text from files for long context  
#     list_of_tokens = []  
...
#     # Get tool-based conversations  
#     dataset = ToolCallDataset("tool_calling")  
#     base_conversations = dataset.get_test_cases(args.num_conversations) 
#     prefix_lengths = args.input_prefix_num_tokens.sample(args.num_conversations)   
      
#     # Add long context to each conversation  
#     conversations = {}  
#     for i, conv in enumerate(base_conversations):  
#         conv_id = f"TOOL_CONV_{i}"  
          
#         # Add long context to user messages  
#         messages = add_long_context(conv["messages"], list_of_tokens, tokenizer, i, prefix_lengths[i])  
          
#         conversations[conv_id] = conv["messages"] #messages  
...
def generate_tool_conversations(args: GenConvArgs, tokenizer) -> ConversationsMap:  
    ...
    # Sample both prefix lengths and output token lengths  
    prefix_lengths = args.input_prefix_num_tokens.sample(args.num_conversations)  
    output_lengths = args.output_num_tokens.sample(args.num_conversations)  # Add this line  
    ...
    for i, conv in enumerate(base_conversations):  
        ...
        # Store output length with conversation for potential use in benchmarking  
        conversations[conv_id] = {  
            "messages": messages,  
            "output_tokens": output_lengths[i]  # Optional: store for tracking  
        }  
      
    return conversations
  
def add_long_context(messages: list, tokens: list, tokenizer, conv_id: int, prefix_len: int = 1024) -> list:  
    """Add long context from text file to user messages."""  
    updated_messages = []  
      
    for msg in messages:  
        if True: #msg["role"] != "assistant":  
            # Add context before the actual content  
            context_start = conv_id * 1000  # Different context per conversation              
            context_end = context_start + prefix_len  # 2000 tokens of context  
              
            if context_end < len(tokens):  
                context_text = tokenizer.decode(tokens[context_start:context_end])  
                updated_content = f"Context: {context_text}\n\n{msg['content']}"  
                updated_messages.append({**msg, "content": updated_content})  
            else:  
...
#     # Sample prefix lengths for each conversation  
#     prefix_lengths = args.input_prefix_num_tokens.sample(args.num_conversations)  
...
#         # Add varying context based on distribution  
#         updated_messages = add_long_context(  
#             messages,   
#             list_of_tokens,   
#             num_tokens=prefix_lengths[i], # Use sampled value 
#             tokenizer=tokenizer,   
#         )  
          
#         conversations[conv_id] = updated_messages  
      
#     return conversations  
  
# def add_long_context(messages, text_tokens, num_tokens, tokenizer):  
#     """Add long context to user messages."""  
#     updated_messages = []  
      
#     for msg in messages:  
#         if True: #msg["role"] == "user":  
#             # Add context before the actual content  
#             if len(text_tokens) > num_tokens:  
#                 context = tokenizer.decode(text_tokens[:num_tokens])  
