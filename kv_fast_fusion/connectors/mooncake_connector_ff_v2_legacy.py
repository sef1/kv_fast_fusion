"""PRE-EXTRACTION Mooncake BFF v2, kept verbatim as a measurement baseline. DO NOT EDIT.

Byte-for-byte `kv_fast_fusion/connectors/mooncake_connector_ff_v2.py` as of commit `6122e3126`,
except for the registration function at the bottom, which registers it as
`MooncakeConnectorFFv2Legacy` so it can live beside the current connector instead of replacing it.

**Why it exists.** Extracting the shared core for the Ascend port (commit `1f6479d0f`) took this
file from **773 lines to 360**, moving the decision logic into `kv_fast_fusion/pd_dedup_v2.py`. That
refactor has never been measured against what preceded it: an A/B run on 2026-08-19 used
`1f6479d0f` as the "before" arm, but `1f6479d0f` *is* the extraction commit, so both arms were
post-extraction and unsurprisingly agreed (1.155/1.171 vs 1.096). The user reports 1.42 req/s from
the pre-extraction code, and this file is the only way to test that claim.

It is safe to host here because everything it depends on is unchanged since `6122e3126`:
`connectors/mooncake_connector_ff.py`, `constants.py` and `fast_fusion_block_pool.py` are all
byte-identical at the branch tip, and `pd_lsh.py` / `pd_dedup_plan.py` were purely added.

Select with `BASELINE=bff_v2_legacy`. If it reproduces 1.42, diff it against the current
360-line connector plus `pd_dedup_v2.py` to find the behaviour the extraction dropped; if it does
not, the difference lies outside this file and this baseline can be deleted.
...
LEGACY_NAME = "MooncakeConnectorFFv2Legacy"


def register_mooncake_connector_ff_v2_legacy() -> None:
    """Register the PRE-EXTRACTION v2 connector under its own name, beside the current one.

    Registered as ``MooncakeConnectorFFv2Legacy`` rather than replacing ``MooncakeConnectorFFv2``
    so both live in one tree and the A/B is a single ``BASELINE=`` apart — no checkout, and the
    current tests keep testing the current code."""
    from vllm.distributed.kv_transfer.kv_connector.factory import KVConnectorFactory
    if LEGACY_NAME in KVConnectorFactory._registry:
        return
    KVConnectorFactory.register_connector(
        LEGACY_NAME,
        "kv_fast_fusion.connectors.mooncake_connector_ff_v2_legacy",
        "MooncakeConnectorFFv2",
    )
    logger.info("Fast fusion P/D patch: registered %s (pre-extraction v2, 773 lines).",
                LEGACY_NAME)
